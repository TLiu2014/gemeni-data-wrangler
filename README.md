...
# Optionally add GEMINI_API_KEY in server/.env as a default
# You can also set your API key in the app: Settings → Gemini API Key
```
...
When you enter a Gemini API key in **Settings** in the UI, the server saves it encrypted so it can be reused after you close the tab or refresh.

- **Where it’s stored:** The key is encrypted and written to `server/data/api-key.enc`. This directory is in `.gitignore` and is not committed.
- **Encryption:** AES-256-GCM. A master key is used to encrypt and decrypt the stored API key.
- **Master key:** On first use, the server generates a random 32-byte master key and writes it to `server/data/secret.key`. That file is created automatically; do not commit it. If you lose `secret.key`, the stored API key cannot be decrypted and must be re-entered in Settings.
- **Resolution order:** For each request, the server uses an API key from (1) the request body if the client sent one, (2) the decrypted value from `api-key.enc`, or (3) `GEMINI_API_KEY` from `server/.env` if set.
